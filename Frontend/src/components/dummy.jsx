...
export default function HowVTSWorks({ steps = [] }) {
  ...
  return (
    <section className="relative bg-[#eeeeee] py-20 overflow-hidden">
      ...
      {/* Bigger Boy Image */}
      <div className="absolute right-[10%] top-2 hidden lg:block z-20">
        <img
          src={BoyImg}
          alt="Student"
      ...
      <div className="mx-auto max-w-4xl">
        ...
        {/* Animated Frame */}
        <div className="vts-frame mt-12 rounded-3xl">

          <div className="relative z-10 px-10 py-12">

            {/* TOP FLOW LINE */}
            <div className="absolute left-[12%] right-[12%] top-4 h-[4px] rounded-full bg-gradient-to-r from-[#6621BA] via-[#FF9C00] to-[#6621BA]" />
                ...
                <div key={s.id} className="relative w-[23%]">
                  ...
                  <div className="relative rounded-xl shadow-[0_15px_25px_rgba(0,0,0,0.25)] overflow-hidden">
                    ...
                    <div className="relative z-10 px-5 py-8 text-center">
                      ...
                      <i className={`bi ${topIcon(idx)} text-3xl text-[#FF9C00]`} />
...
export default function EnrollModal({ open, onClose, course, coursesList = [] }) {
  ...
  const courseAmount = Number(
    selectedCourseObj?.course_fee || selectedCourseObj?.course_fee === 0
  ...
  const confirmDetails = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${base}/api/enrollments/${enrollment.id}/confirm/`, {
        method: "POST",
      });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 p-4" onClick={onClose}>
      <div
        className="mx-auto w-full max-w-4xl overflow-hidden rounded-[30px] bg-white shadow-[0_25px_80px_rgba(0,0,0,0.35)]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* header / stepper */}
        <div className="relative px-8 pt-6">
          <button
            className="absolute right-5 top-5 flex h-9 w-9 items-center justify-center rounded-full border border-black/40 text-lg"
            onClick={onClose}
            aria-label="Close"
          >
            ✕
        ...
        {/* content */}
        <div className="max-h-[72vh] overflow-auto px-10 pb-8 pt-5">
          {err ? (
            <div className="mb-5 rounded-xl bg-red-50 p-3 text-red-700">{err}</div>
          ) : null}
...
function Stepper({ current }) {
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      <Pill
        label="Enter Details"
        variant={enterDone ? "done" : current === 0 ? "active" : "idle"}
      />
      <Line active={enterDone} />

      <Pill
        label="Confirm Details"
        variant={confirmDone ? "done" : current === 1 ? "active" : "idle"}
      />
      <Line active={confirmDone} />

      <Pill
        label="Payment"
        variant={current >= 2 ? (current === 2 ? "active" : "done") : "idle"}
      />
    </div>
  );
}
...
function Pill({ label, variant }) {
  ...
  return (
    <div
      className={`flex min-w-[190px] items-center justify-center rounded-full px-8 py-3 text-base font-semibold ${cls}`}
    >
      {label}
      {variant === "done" ? (
        <span className="ml-3 inline-flex h-7 w-7 items-center justify-center rounded-full bg-white text-[#3E9B43] font-extrabold">
          ✓
        </span>
      ) : null}
    </div>
...
function Title({ children }) {
  return <h2 className="text-center text-3xl font-semibold text-black">{children}</h2>;
}
...
function Input(props) {
  return (
    <input
      {...props}
      className={
        "w-full rounded-xl border-2 border-[#B9B9B9] px-4 py-3 text-base outline-none focus:border-[#6621BA] " +
        (props.className || "")
      }
    />
  );
...
function Select(props) {
  return (
    <select
      {...props}
      className={
        "w-full rounded-xl border-2 border-[#B9B9B9] bg-white px-4 py-3 text-base outline-none focus:border-[#6621BA] " +
        (props.className || "")
      }
    />
  );
    ...
    <>
      ...
      <div className="mt-8 flex justify-end">
        <button
          onClick={onSubmit}
          disabled={loading}
          className="rounded-2xl bg-[#FF9C00] px-9 py-3 text-base font-semibold text-white disabled:opacity-60"
        >
...
function ConfirmDetails({ enrollment, courseTitle, onEdit, onConfirm, loading }) {
  ...
  return (
    <>
      ...
      <div className="mx-auto mt-8 max-w-4xl rounded-2xl bg-[#EFE6FF] p-8">
      ...
      <div className="mt-10 flex justify-center gap-8">
        <button
          onClick={onEdit}
          disabled={loading}
          className="min-w-[190px] rounded-2xl border border-[#FF9C00] px-8 py-3 text-base font-semibold"
        >
          Back to Edit
        </button>
        <button
          onClick={onConfirm}
          disabled={loading}
          className="min-w-[190px] rounded-2xl bg-[#FF9C00] px-8 py-3 text-base font-semibold text-white disabled:opacity-60"
        >
      ...
      <div className="mt-8 grid gap-10 md:grid-cols-2">
        ...
        <div className="space-y-6">
          <PayOption
            ...
            icons={
              <div className="flex items-center gap-4">
                ...
                <div className="flex h-9 w-9 items-center justify-center rounded bg-[#5B2DAA] text-white font-extrabold text-lg">
                  पे
                </div>
              </div>
            }
          ...
          <PayOption
            ...
            title="Net Banking"
            icons={<i className="bi bi-bank2 text-3xl text-[#2B4C92]" />}
          />
          <PayOption
            ...
            icons={
              <div className="flex items-center gap-5">
                ...
                <span className="text-2xl font-extrabold text-[#1A4DBA]">VISA</span>
              </div>
            }
          />
          ...
          <div className="pt-2 flex justify-end">
            <button
              onClick={onPay}
              disabled={loading}
              className="min-w-[190px] rounded-2xl bg-[#FF9C00] px-8 py-3 text-lg font-semibold text-white disabled:opacity-60"
            >
...
function PayOption({ active, onClick, title, icons }) {
  return (
    <button
      onClick={onClick}
      className={`w-full rounded-2xl border px-6 py-5 text-left ${
      ...
      <div className="flex items-center gap-5">
        <div
          className={`h-6 w-6 rounded-full border-2 ${
            active ? "border-[#FF9C00]" : "border-black/40"
          } flex items-center justify-center`}
        >
          {active ? <div className="h-3 w-3 rounded-full bg-[#FF9C00]" /> : null}
        </div>
  ...
  return (
    <>
      <div className="flex flex-col items-center pt-4">
        <div className="flex h-20 w-20 items-center justify-center rounded-full bg-[#3E9B43] text-white text-4xl font-extrabold">
          ✓
        </div>

        <div className="mt-5 text-3xl font-semibold">Payment Completed</div>

        <div className="mt-7 w-full max-w-xl rounded-2xl bg-[#DDEEDC] p-7">
          <div className="text-center text-2xl font-semibold mb-5">Transaction Details</div>
        ...
        <div className="mt-8 flex gap-4">
          {payment.invoice_url ? (
            <a
              ...
              download
              className="rounded-2xl bg-[#FF9C00] px-10 py-3 text-lg font-semibold text-white"
            >
              Download Invoice
            </a>
          ) : null}
          ...
          <button
            onClick={onClose}
            className="rounded-2xl border border-black/20 px-9 py-3 text-lg font-semibold"
          >
            Close
          </button>
        </div>
...
function TxRow({ k, v }) {
  return (
    <div className="flex items-center justify-between gap-6 py-2 text-lg">
      <div className="w-[170px]">{k}</div>
      <div className="font-medium">:</div>
